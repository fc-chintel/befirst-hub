...
function svgIcon(slug) {
  const d = SNS_ICON[slug];
  return d ? `<svg viewBox="0 0 24 24" fill="currentColor"><path d="${d}"/></svg>` : "";
}
...
function snsButtons(m) {
  const btns = [];
  if (m.sns?.instagram)
    btns.push(`<a class="sns-btn" href="${esc(m.sns.instagram.url)}" target="_blank" rel="noopener">${svgIcon("instagram")}Instagram</a>`);
  if (m.sns?.x)
    btns.push(`<a class="sns-btn" href="${esc(m.sns.x.url)}" target="_blank" rel="noopener">${svgIcon("x")}X</a>`);
  if (m.sns?.tiktok)
    btns.push(`<a class="sns-btn" href="${esc(m.sns.tiktok.url)}" target="_blank" rel="noopener">${svgIcon("tiktok")}TikTok</a>`);
...
function metaCell(label, value) {
  return `<div class="meta-cell"><span class="mk">${label}</span><span class="mv">${value}</span></div>`;
}
...
function renderMembers(members) {
  ...
  box.innerHTML = members.map((m) => {
    ...
    const avatar = m.photo
      ? `<img src="${esc(m.photo)}" alt="${esc(m.name)}" loading="lazy" onerror="this.replaceWith(Object.assign(document.createElement('span'),{className:'initials',textContent:'${initials}'}))">`
      : `<span class="initials">${initials}</span>`;

    const cells = [];
    if (m.birthday)
      cells.push(metaCell("BIRTHDAY", `${fmtBirthday(m.birthday)}${bd !== null ? `<small>あと${bd}日</small>` : ""}`));
    if (m.hometown) cells.push(metaCell("FROM", esc(m.hometown)));
    if (m.height) cells.push(metaCell("HEIGHT", esc(m.height)));
    if (m.role) cells.push(metaCell("STYLE", esc(m.role)));

    const sns = snsButtons(m);

    return `
    <article class="member-card" style="--mc:${esc(m.color || "#e8bd63")}">
      ...
      ${m.intro ? `<p class="member-intro">${esc(m.intro)}</p>` : ""}
      ${cells.length ? `<div class="member-meta">${cells.join("")}</div>` : ""}
      ${sns ? `<div class="sns-row">${sns}</div>` : ""}
      ${m.trivia?.length ? `
      <details class="trivia">
        <summary>知る人ぞ知る話 <span class="trivia-count">${m.trivia.length}</span></summary>
        <ul>${m.trivia.map((t) => `<li>${esc(t.text)}${t.source ? `<small class="trivia-src">出典: ${esc(t.source)}</small>` : ""}</li>`).join("")}</ul>
      </details>` : ""}
    </article>`;
  }).join("");
...
function revealOnScroll() {
  ...
  document.querySelectorAll(".about-card, .member-card").forEach((el, i) => {
